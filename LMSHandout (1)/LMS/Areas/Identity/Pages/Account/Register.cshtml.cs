...
namespace LMS.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        /// </summary>
        /// <param name="role">The role for which to generate the id</param>
        /// <returns>The unique uid</returns>
        string GenerateUid(string role)
        {
            ...
            while (uid == null)
            {
                ...
                int randomNum = rnd.Next(1000000, 10000000);
                string possibleUid = "u" + randomNum.ToString();
                bool exists = false;

                if (role.Equals("Administrator"))
                {
                    exists = db.Administrators.Any(a => a.UId == possibleUid);
                }

                else if (role.Equals("Professor"))
                {
                    exists = db.Professors.Any(p => p.UId == possibleUid);
                }

                else
                {
                    exists = db.Students.Any(s => s.UId == possibleUid);
                }
